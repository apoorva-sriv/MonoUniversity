...
function chTTCDouble(playerNum) {
    ...
    if (player.position == chanceTiles[0]) station_position = 15;

    if (player.position == chanceTiles[1]) station_position = 25;

    if (player.position == chanceTiles[2]) station_position = 5;
...
function chRepairs(playerNum) {
    ...
    for (let i = 0; i < gameBoard.tiles.length; i++) {
        if (gameBoard.tiles[i].tileflags != TILE_FLAG_NORMAL) continue;

        if (gameBoard.tiles[i].owner != playerNum) continue;

        if (gameBoard.tiles[i].building != true) continue;
...
function chMoveUtility(playerNum) {
    ...
    if (player.position == chanceTiles[0] || player.position == chanceTiles[2]) utility_position = utilityTiles[0];
    else utility_position = utilityTiles[1];
...
function chGOOJFC(playerNum) {
    ...
    if (gameBoard.players[playerNum].piece == PIECE_TYPE_LAWYER) gameBoard.players[playerNum].jailcards += 2;
...
function initializePlayers(board, numPlayers) {
    ...
    for (let i = 0; i < board.playerTurns.length; i++) {
        ...
        let imgPathString =
            "url('./img/pieces/" +
            pieceGraphics[board.players[board.playerTurns[i]].piece] +
            "-" +
            board.players[board.playerTurns[i]].color +
            ".png')";
...
function getTileInfo(tile, cardNum) {
    switch (tile.tileflags) {
        ...
        case TILE_FLAG_COMMUNITY:
            if (cardNum <= -1) return "Community Chest Card!";
            else return gameBoard.communityCards[cardNum][0];
            break;
        case TILE_FLAG_CHANCE:
            if (cardNum <= -1) return "Chance Card!";
            else return gameBoard.chanceCards[cardNum][0];
            break;
        case TILE_FLAG_JAIL:
...
function landedTileInfo(playerNum) {
    ...
    if (tileInfo && tile) {
        ...
        else infoTileText.innerHTML = getTileInfo(tileInfo, -1);
...
function playerEffects() {
    ...
    if (player.jailed == true) gameBoard.timeOutId = setTimeout(nextTurn, 2000);
    else gameBoard.timeOutId = setTimeout(playerDecisionTime, 2000);
    ...
    if (player.user) {
        ...
        if (player.aiprofile != AI_PROFILE_TEST) {
            ...
            if (checkCanBuy(playerNum, player.position)) {
                if (
                    player.aiprofile == AI_PROFILE_AGGRESSIVE ||
                    (player.aiprofile == AI_PROFILE_RANDOM && Math.floor(Math.random() * 2) == 1)
                )
...
function nextTurn() {
    ...
    if (gameBoard.dice[0] != gameBoard.dice[1]) {
        if (gameBoard.playerTurn + 1 >= gameBoard.playerTurns.length) gameBoard.playerTurn = 0;
    ...
    if (checkFunds(checkPlayer)) {
        ...
        if (gameBoard.playerTurns.length <= 1) return;
    }
...
function checkFunds(playerNum) {
    if (gameBoard.players[playerNum].money >= 0) return true;
...
function kickPlayer(playerNum) {
    ...
    if (!gameBoard.playerTurns.includes(playerNum)) return;

    // Do not kick if we only have one left
    if (gameBoard.playerTurns.length < 2) return;
    ...
    if (index == playerTurn) removingOther = false;

    // Adjust turn and remove from playerTurns
    if (gameBoard.playerTurns.length == 2) {
        gameBoard.playerTurn = 0;
    } else {
        if (index < playerTurn) gameBoard.playerTurn -= 1;
    }

    // Reset index to a proper value
    if (gameBoard.playerTurn >= gameBoard.playerTurns.length - 1) gameBoard.playerTurn = 0;
    ...
    if (removedPlayer.user) lowlightDice();
...
function checkWinCondition() {
    if (gameBoard.playerTurns.length <= 1) return true;
...
function tileLand(playerNum, tileNum) {
    ...
    if (playerPtr.passedgo == true) {
        if (playerPtr.gorestrict == true) {
            ...
            if (playerPtr.piece == PIECE_TYPE_VETERAN) loot -= 50;

            if (playerPtr.piece == PIECE_TYPE_WORKER) loot += 50;
    ...
    if (tileFlags == TILE_FLAG_NORMAL || tileFlags == TILE_FLAG_UTILITY || tileFlags == TILE_FLAG_TTC) {
        if (tile.owner != playerNum && tile.owner != null) {
            ...
            if (tileFlags == TILE_FLAG_UTILITY && playerPtr.piece != PIECE_TYPE_GENIUS) {
                ...
                if (playerPtr.piece == PIECE_TYPE_VETERAN) loot *= 0.9;

                if (playerPtr.piece == PIECE_TYPE_BANKER) loot *= 1.1;
        ...
        if (playerPtr.piece == PIECE_TYPE_WORKER || playerPtr.piece == PIECE_TYPE_GENIUS) loot *= 1.25;
...
function hasSufficentFunds(playerNum, fundsAmount) {
    if (gameBoard.players[playerNum].piece == PIECE_TYPE_LAWYER) fundsAmount += fundsAmount / 10;
...
function checkCanBuy(playerNum, tileNum) {
    ...
    if (gameBoard.tiles[tileNum].purchaseable == false) return false;

    if (gameBoard.tiles[tileNum].owner != null) return false;

    if (gameBoard.players[playerNum].piece == PIECE_TYPE_INVESTOR) {
        if (!hasSufficentFunds(playerNum, gameBoard.tiles[tileNum].price * 1.5)) return false;
    } else {
...
function updatePlayerList() {
    ...
    for (let i = 0; i < gameBoard.playerTurns.length; i++) {
        ...
        if (actualPlayerId == 0) {
            ...
            if (login.isAdmin) {
                ...
                if (gameBoard.playerTurns[gameBoard.playerTurn] != 0) playerKickButton.setAttribute("disabled", "");
    ...
    if (gameBoard.players[gameBoard.playerTurns[gameBoard.playerTurn]].user)
        curPlayer = document.createTextNode(
            gameBoard.players[gameBoard.playerTurns[gameBoard.playerTurn]].user.username
        );
    else curPlayer = document.createTextNode("AI " + gameBoard.playerTurns[gameBoard.playerTurn]);
...
function drawChanceCard(playerNum) {
    ...
    gameBoard.timeOutId = setTimeout(function () {
        clearTimeout(gameBoard.timeOutId);
        gameBoard.timeOutId = setTimeout(nextTurn, 2000);
        gameBoard.chanceCards[gameBoard.chanceCount][1](playerNum);
    }, 2000);
}
...
function drawCommunityCard(playerNum) {
    ...
    gameBoard.timeOutId = setTimeout(function () {
        clearTimeout(gameBoard.timeOutId);
        gameBoard.timeOutId = setTimeout(nextTurn, 2000);
        gameBoard.communityCards[gameBoard.communityCount][1](playerNum);
    }, 2000);
}
...
function purchaseTile(playerNum, tileNum) {
    ...
    if (playerNum > -1) {
        ...
        if (
            gameBoard.players[playerNum].piece == PIECE_TYPE_INVESTOR &&
            gameBoard.players[playerNum].pastfirst == false
        )
            flowFunds(playerNum, gameBoard.tiles[tileNum].price * 1.5, false);
        else flowFunds(playerNum, gameBoard.tiles[tileNum].price, false);
    } else {
...
function updateOtherInformation() {
    ...
    for (let i = 0; i < gameBoard.playerTurns.length; i++) {
        ...
        if (gameBoard.players[gameBoard.playerTurns[i]].jailed == true) {
            ...
            if (player.user) {
                const playerSlotText = document.createTextNode(
                    player.user.username + " - " + player.jailturns + " |" + " (" + player.jailcards + " )"
                );
                playerSlot.appendChild(playerSlotText);
            } else {
                const playerSlotText = document.createTextNode(
                    "AI " + actualPlayerId + " - " + player.jailturns + " |" + " (" + player.jailcards + " )"
                );
                playerSlot.appendChild(playerSlotText);
            }
    ...
    if (!canUseGTFO(0)) gtfoButton.setAttribute("disabled", "");
    else gtfoButton.setAttribute("onclick", "freedom(event)");
...
function canUseGTFO(playerNum) {
    ...
    if (!gameBoard.playerTurns.includes(playerNum)) return false;

    if (player.jailed == false) return false;

    if (player.jailcards < 1) return false;
